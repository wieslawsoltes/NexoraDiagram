...
import { DRAW_TOOLS, DRAW_STYLE, handlePoints, pathControls, pathGeometry, flattenPath, simplifyPath, constrainPoint, resizeGeometry, rotatePoint, strokeDash, geometryBounds } from '../core/drawing.js';
...
const names = { line: 'Line', arrow: 'Arrow', rectangle: 'Rectangle', ellipse: 'Ellipse', polyline: 'Polyline', polygon: 'Polygon', pencil: 'Freehand', arc: 'Arc', bezier: 'Bézier' };
const symbols = { line: '╱', arrow: '↗', rectangle: '▭', ellipse: '◯', polyline: '⌁', polygon: '⬠', pencil: '✎', arc: '⌒', bezier: '∿' };
...
export function installDrawing(a) {
  ...
  function finishDraw(points, mode = 'linear', closed = false, tool = 'line') {
  ...
  function finishDraft() {
    ...
    finishDraw(d.points, 'linear', d.tool === 'polygon', d.tool);
  }
  ...
  function move(e, world, schedule = true) {
    ...
    try {
      if (d.kind === 'edit-draw') {
        ...
        if (d.tool === 'pencil' && distance(d.points.at(-1), world) * a.camera.zoom > 1 && d.points.length < 4096) d.points.push({ ...world });
      ...
      if (d.kind === 'edit-move') {
        ...
        Object.assign(a.page.view.nodes[d.id], pathGeometry(points, d.pathOriginal.pathMode, d.pathOriginal.closed));
      }
  ...
  function up(e, world, d) {
    ...
    try {
      if (d.kind === 'edit-draw') {
        ...
        const points = d.tool === 'pencil' ? simplifyPath(d.points, .6 / a.camera.zoom) : drawPoints(d, d.current);
        finishDraw(points, d.tool === 'arc' ? 'quadratic' : d.tool === 'bezier' ? 'cubic' : 'linear', false, d.tool);
  ...
  function down(e, world) {
    if (DRAW_TOOLS.includes(a.tool)) {
      ...
      if (['polyline', 'polygon'].includes(a.tool)) {
        if (!draft) draft = { tool: a.tool, points: [p], current: p };
          ...
          draft.points.push(constrainPoint(draft.points.at(-1), p, e.shiftKey)); draft.current = p;
        }
        a.requestFrame(); return true;
      }
      a.drag = { kind: 'edit-draw', tool: a.tool, start: p, current: p, points: [p] }; return true;
    }
  ...
  Object.assign(cmd, {
    ...
    'delete-point': () => {
      ...
      a.store.transact('Delete path point', () => Object.assign(g, pathGeometry(points, 'linear', g.closed))); pointIndex = -1;
    },
  ...
  function insertPoint(p) {
    ...
    a.store.transact('Insert path point', () => Object.assign(g, pathGeometry(points, 'linear', g.closed))); pointEdit = true; pointIndex = index; a.requestFrame();
  }
  ...
  a.editor = {
    ...
    overlay() {
      ...
      const id = selectedPath(); if (id) {
        const ps = pathControls(a.page.view.nodes[id]); if (pointEdit) line(ps, 'stroke-dasharray="3 3"');
